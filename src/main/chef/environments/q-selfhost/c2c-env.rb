name "c2c-env"

# ROLE for Code2Cloud Dev environment properties (https://q.tasktop.com/alm)

override_attributes \
:java => {
  :java_home => "/usr/lib/jvm/java-6-sun/jre"
},
:tomcat => {
  :second_service => true
},
:c2c => {
  :server => {
      :s3 => {
        :access_key_id => "AKIAI6FIC7DBENCWERKA"
      },
  },
  :hosts => {
     "hub"           => { :ipaddress => "192.168.51.27", :description => "Hub node" },
     "nexus"         => { :ipaddress => "192.168.51.27",  :description => "Nexus Repository" },
     "db"            => { :ipaddress => "192.168.51.27",  :description => "Database Server" },
     "dmz"           => { :ipaddress => "192.168.51.27",   :description => "DMZ node" },
     "hudson_master" => { :ipaddress => "192.168.51.27",  :description => "Hudson Master" },
     "hudson_slave"  => { :ipaddress => "192.168.51.32",  :description => "Hudson Slave" },
     "scm"           => { :ipaddress => "192.168.51.27",  :description => "SCM node" },
     "task"          => { :ipaddress => "192.168.51.27", :description => "Task node" },
     "profile"       => { :ipaddress => "q.tasktop.com", :description => "Public facing profile host" },
   },
  :github => {
    :consumer_key => "a30115eb45f34f83a4a5"
  },
  :hub => {
    :prefix_path => "/alm",
    :invitation_only => true,
    :remember_me_key => "p928375614k6j/14m43iotgha''423j56;11",
    :has_internal_services => true,
    :additional_properties => [
      "alm.hudsonSlave.build.directoriesToClean=/tmp",
      "alm.databaseBacked.maxTenantsPerNode=100",
      "alm.diskBacked.maxTenantsPerNode=100",
      "alm.hudsonMaster.maxTenantsPerNode=100"
    ]
  },
  :hudson => {
    :update_plugins => false # We have custom gerrit plugin enabled for c2c project
  },
  :nexus => {
    :port => "7070"
    },
  :authorized_keys => [{
    :name => "vcloud",
      :keys => ["ssh-rsa  cmorgan",
                 "ssh-rsa  lucas"]
  }]
}
